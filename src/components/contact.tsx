...
export default function ContactUs() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch(
        "https://varunportfolio-one.vercel.app/api/contact",
        {
          method: "POST",
          headers: {
  ...
  return (
    <div className="relative container mx-auto px-4 py-8">
      {/* <span className="font-caveat absolute left-auto bottom-0 z-[-1] w-100 text-[250px] md:text-[350px] opacity-5 font-semibold leading">
        contact
      </span> */}
      <h1 className="text-5xl font-semibold text-gray-700 dark:text-darkprimary uppercase my-5 relative">
        Contact
      </h1>
      <p className="font-caveat font-semibold text-2xl my-5">
        ...
        <div className="md:col-span-5 gap-3">
          {data?.map((datas, index) => (
            <div key={index} className="flex items-center my-8 2 gap-3">
              ...
              <div>
                <p className="font-semibold font-caveat text-2xl">
                  {datas.name}
                </p>
          ...
          <form onSubmit={handleSubmit}>
            <div className="md:flex gap-14">
              <div>
                ...
                <input
                  type="text"
                  name="name"
                  className="my-2 border-[1px] rounded-full p-2 dark:bg-transparent"
            ...
            <div>
              ...
              <input
                type="text"
                name="subject"
                className="my-2 border-[1px] rounded-full p-2 md:w-5/6 dark:bg-transparent"
                onChange={handleChange}
                required
              />
            </div>
            <div>
              ...
              <br />
              <textarea
                className="my-2 border-[1px] rounded-xl p-2 md:w-5/6 dark:bg-transparent"
                rows={4}
                name="message"
                onChange={handleChange}
                required
            ...
            <button
              type="submit"
              className="mx-auto bg-primary dark:bg-darkprimary p-2 rounded-md text-white w-32"
            >
              Send
            </button>
          </form>
